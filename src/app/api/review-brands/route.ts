...
export async function GET(req: NextRequest) {
  ...
  const items = await prisma.reviewBrand.findMany({ where, orderBy: { createdAt: 'desc' } })
...
export async function POST(req: NextRequest) {
  try {
    ...
    const exists = await prisma.reviewBrand.findUnique({ where: { slug } })
    if (exists) return NextResponse.json({ error: 'Slug zaten mevcut' }, { status: 400 })
    const created = await prisma.reviewBrand.create({
