...
export default function CreateLinkButton() {
  ...
  async function submit() {
    try {
      ...
      const res = await fetch('/api/affiliate-links', {
        ...
        body: JSON.stringify({ title, targetUrl, slug: slug || undefined }),
      })
  ...
  return (
    <div>
      <button className="px-3 py-2 rounded bg-primary text-primary-foreground hover:opacity-90" onClick={() => setOpen(true)}>Yeni Link Oluştur</button>
      {open && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-card text-card-foreground border border-border rounded-lg shadow p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">Yeni Link</h3>
            <div className="space-y-3">
              ...
              <div>
                <label className="text-sm text-muted-foreground">Kısa URL (slug) - opsiyonel</label>
